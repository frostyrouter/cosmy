...
Routers write observations only for the exact locally known canary version, update local admission immediately after their own outcome, and poll durable rollout state with the registry refresh loop for cross-instance convergence. Observations use a dedicated four-connection PostgreSQL bulkhead with 100 ms server statement deadlines, 75 ms lock deadlines, and a 200 ms client query deadline; reservations, idempotency, readiness, registry refresh, and administration keep the primary pool. Server-confirmed lock or statement cancellations retry in fresh transactions with bounded jitter, preserving exactly one committed counter update while absorbing short contention bursts. The response path itself still waits at most 100 ms. Metrics expose `rollout_auto_rollback` and `rollout_observation_failure`; persistent timeout or storage failure never replaces the caller's provider result.
