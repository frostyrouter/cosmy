...
export class InMemoryHealthStore implements HealthSnapshotStore {
  ...
  markSuccess(modelId: string, latencyMs: number): void { this.successes.set(modelId, (this.successes.get(modelId) ?? 0) + 1); this.latencies.set(modelId, latencyMs); this.updated.set(modelId, nowIso()); }
  markFailure(modelId: string): void { this.failures.set(modelId, (this.failures.get(modelId) ?? 0) + 1); this.updated.set(modelId, nowIso()); }
