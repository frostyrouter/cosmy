...
describe.skipIf(!databaseUrl)('PostgreSQL budget integration', () => {
  ...
  beforeEach(async () => {
    await db.query("DELETE FROM usage_reservations WHERE tenant_id = 'budget-tenant'");
    await db.query("DELETE FROM tenant_budgets WHERE tenant_id = 'budget-tenant'");
    await repository.setBudget('budget-tenant', 0.01);
  });
  ...
  it('atomically rejects concurrent reservations that would overspend', async () => {
    const attempts = await Promise.allSettled([
      repository.reserve({ tenantId: 'budget-tenant', estimatedCostUsd: 0.007 }),
      repository.reserve({ tenantId: 'budget-tenant', estimatedCostUsd: 0.007 }),
    ]);
    expect(attempts.filter((attempt) => attempt.status === 'fulfilled')).toHaveLength(1);
    const rejected = attempts.find((attempt) => attempt.status === 'rejected');
    expect(rejected).toMatchObject({ reason: { code: 'budget_exceeded', statusCode: 429 } });
    expect(await repository.usageFor('budget-tenant')).toEqual({ reservedUsd: 0.007, spentUsd: 0 });
  });

  it('reconciles exactly once and releases capacity', async () => {
    const reservation = await repository.reserve({ tenantId: 'budget-tenant', estimatedCostUsd: 0.007 });
    await repository.reconcile(reservation, 0.004);
    await repository.reconcile(reservation, 0.009);
    expect(await repository.usageFor('budget-tenant')).toEqual({ reservedUsd: 0, spentUsd: 0.004 });
    await expect(repository.reserve({ tenantId: 'budget-tenant', estimatedCostUsd: 0.006 })).resolves.toBeTruthy();
  });
  ...
  it('conservatively charges and releases an abandoned reservation', async () => {
    ...
    expect(await repository.usageFor('budget-tenant')).toEqual({ reservedUsd: 0, spentUsd: 0.007 });
